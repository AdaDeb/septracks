        ...
        try {
          ...
          for(GamificationAchievement a : achievements){
            if(isPassedAchievement(a)){
              SharedPreferences sp = GamificationService.this.getSharedPreferences(Constants.SETTINGS_NAME, Context.MODE_PRIVATE);
              boolean isCompleted = sp.getBoolean("completed-" + a.getId(), false);
              if(!isCompleted){ 
...
private void setChallengeCompleted(int id){
    SharedPreferences sp = this.getApplicationContext().getSharedPreferences(Constants.SETTINGS_NAME, Context.MODE_WORLD_WRITEABLE);
